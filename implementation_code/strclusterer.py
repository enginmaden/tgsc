...
def process_first_window(file_in):
    ...
    window = [next(file_in) for x in range(batch_size * window_size)]
    for line in window:
        splitted=line.split(':')
    ...
    print_time()
    generate_sbert_emb(tweets,0,(batch_size * window_size))
...
while True:
    try:
        ...
        print("processing tweets ",last_processed_ind,"->",last_processed_ind+(window_size * batch_size))
        print_time()
...
if len(batch) > 0:        
    ...
    tweets=tweets[batch_size:]
    removed_batch_count+=1
    print("processing tweets ",last_processed_ind,"->",last_processed_ind+((window_size -1) * batch_size)+len(batch))
    print_time()
    # Generate vectors from embedding with S-BERT for the new incoming batch        
